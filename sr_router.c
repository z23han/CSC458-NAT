...
*/
        ...
        /* ************check the nat and forward the packet************ */
            ...
            /* else if it is tcp */
				...
				if ((ntohs(tcp_hdr->dst_port) >= 0 && ntohs(tcp_hdr->dst_port) <= 1023) || (ntohs(aux_int) >= 0 && ntohs(aux_int) <= 1023) || 
				(ntohs(aux_ext) >= 0 && ntohs(aux_ext) <= 1023)) {
        ...
        /* ********sent to me and is coming from external and is sent to the router******** */ 
            ...
            /* else if it is tcp */
				...
				/* check if the port number is between 0~1023 */
				if ((ntohs(tcp_hdr->src_port) >= 0 && ntohs(tcp_hdr->src_port) <= 1023) || (ntohs(aux_int) >= 0 && ntohs(aux_int) <= 1023) || 
				(ntohs(aux_ext) >= 0 && ntohs(aux_ext) <= 1023)) {
